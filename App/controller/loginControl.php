...
class loginControl {
   private $message = array(),
    ...
    function authentication($username, $password) {
        ...
        if (!$this->isAuthenticated($username, sha1($password))) {
    ...
    function isAuthenticated($username, $password) {
        ...
        $result = R::getAll( 'SELECT * FROM buyer WHERE email = :username and password = :password',
    ...
    function signUpValidate($userName, $password, $confirmpasswd, 
            ...
            if(empty($error_msg)) {
               //put the data in the data base
                ...
                $saveStatus = $this->addUser($userName, $password, $confirmpasswd, 
                            $firstname, $lastname, $address, 
                            $homephone, $cellphone, $identity);
    ...
    function addUser($userName, $password, $confirmpasswd, 
                            $firstname, $lastname, $address, 
                            $homephone, $cellphone, $identity){
            try{
                $user = R::dispense('buyer');
    ...
    function isNotInDb ($email) {
        
        $user = R::findOne('buyer', 'email=?', array($email));
         if($user != null) {
